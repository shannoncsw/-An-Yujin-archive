...
const MediaGrid = ({ mediaUrls }) => {
  ...
  // Safely stop if there's no media array or if it's empty
  ...
  return (
    <div className="mt-4">
      {/* 1. INSTAGRAM GRID (Capped at 3 items visually) */}
        ...
        {mediaUrls.slice(0, 3).map((url, index) => {
          ...
          return (
            <div 
              ...
              className="relative aspect-square bg-neutral-800 rounded-lg overflow-hidden cursor-pointer group border border-neutral-700/50 hover:border-purple-500 transition-all duration-200"
            >
              {/* Thumbnail */}
              <img 
                src={thumbnailUrl} 
              ...
              {/* YouTube Play Overlay (Only on non-masked items) */}
              {ytId && !isLastVisible && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/30">
                  <div className="bg-purple-600 p-2.5 rounded-full text-white shadow-lg transform group-hover:scale-110 transition duration-200">
              ...
              {isLastVisible && remainingCount > 0 && (
                <div className="absolute inset-0 bg-black/75 backdrop-blur-[1px] flex flex-col items-center justify-center text-white font-bold transition group-hover:bg-black/60">
                  <span className="text-lg sm:text-xl">+{remainingCount}</span>
                  <span className="text-[10px] uppercase tracking-wider text-neutral-300">More</span>
                </div>
              )}
            </div>
          );
      ...
      {/* 2. THE LIGHTBOX SLIDERSHOW MODAL */}
      {activeIndex !== null && (
        <div 
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4 backdrop-blur-md"
          onClick={() => setActiveIndex(null)} // Close when clicking backdrop
        >
          {/* Close Button */}
          <button 
            className="absolute top-4 right-4 text-neutral-400 hover:text-white text-4xl font-light p-2 z-50 transition"
          ...
          {activeIndex > 0 && (
            <button 
              onClick={handlePrev}
              className="absolute left-4 z-50 bg-neutral-900/60 hover:bg-neutral-800 text-white p-3 rounded-full text-xl transition"
            >
              &#10094;
            </button>
          )}
          ...
          {activeIndex < mediaUrls.length - 1 && (
            <button 
              onClick={handleNext}
              className="absolute right-4 z-50 bg-neutral-900/60 hover:bg-neutral-800 text-white p-3 rounded-full text-xl transition"
            >
              &#10095;
            </button>
          )}
          ...
          <div 
            className="relative w-full max-w-4xl aspect-video max-h-[80vh] flex items-center justify-center bg-black rounded-xl overflow-hidden shadow-2xl"
            onClick={(e) => e.stopPropagation()} // Keeps click from closing modal
          >
